...
- **`DeterminantPortfolioSelector`** (`gpp_portfolio.py`) — greedy portfolio assembly via incremental correlation-distance maximization. Balances EV and lineup diversity by selecting each successive lineup to maximize `(1 - r) / 2`, the normalized correlation-distance between the candidate's payout and the running portfolio's aggregate payout (sum of already-selected payout vectors) — monotonically decreasing in signed correlation `r`, not `|r|`, so an anti-correlated (hedge) candidate scores strictly above an uncorrelated one, which scores above a co-correlated one. `gpp.selector_score: tail` makes the EV term rank by the fresh tail currency (`ev_override`), with the first `ceil(gpp.cash_anchor_fraction × size)` picks still on mean dollar EV; the floor cull, diversity term, and reported EVs stay on mean dollar EV.
