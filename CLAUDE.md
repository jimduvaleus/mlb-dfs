# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## Target device

The web UI is **desktop-only**. No consideration needs to be given to mobile or tablet layouts.

## Long-running / multi-slate scripts

Any script that runs for more than a few seconds, or iterates over multiple slates/contests/other large computational blocks, must:

- Force line-buffered stdout (`sys.stdout.reconfigure(line_buffering=True)`) near the top of the script. Python falls back to full block buffering once stdout isn't a TTY, so a script redirected to a log file (the normal way to run anything long) would otherwise show nothing in `tail -f` until the process exits.
- Checkpoint after each slate/block: append results to a persistent CSV/file immediately and skip already-completed blocks on the next invocation, so an interrupted run (Ctrl-C, kill, crash) can be resumed with the same command line instead of restarting from scratch. See `tests/backtest.py::_append_and_reload` and `scripts/eval_self_play_selector.py::_append_and_reload`/`_done_slates` for the established pattern.

## Memory-conscious matrix operations

At production scale (`n_sims` ~25,000; pools of tens of thousands of lineups), a single `(n_sims × N)` or `(M × n_sims)` float array is already large enough to matter — e.g. a `(n_sims=25,000 × N=29,411)` float32 field-score array is ~2.9GB — and it's easy to build several of these per iteration of a loop without noticing until the process is swapping. Chunk matrix ops over whichever axis is large (candidates/lineups or sim worlds) instead of materializing the whole array at once. Established precedent: `ContestScorer`'s `candidate_batch_size` and `_PWIN_FIELD_CAP` (`src/optimization/gpp_portfolio.py` / `src/api/external_pool.py`), and `accumulate_currencies`'s `chunk` parameter (`tests/bt_core.py`) for the same idea applied to sim-world chunking. A step that rebuilds one of these arrays every round of a repeated loop (not just once) needs this discipline even more, since the cost multiplies by iteration count — this is exactly what caught out the first self-play selector eval run (`src/optimization/self_play.py`'s `_merge_and_sort_field`/`ctx.scores`, diagnosed via `scripts/profile_self_play_round_cost.py` after the run was killed at RSS ~11.5GB with system swap 90% full).

## Commands

All commands must be run inside the virtualenv. The venv directory is named
`.venv/` on some machines and `venv/` on others (both are gitignored), so use
whichever exists:

```bash
source .venv/bin/activate   # or: source venv/bin/activate
```

**Run all tests:**
```bash
python -m pytest tests/ --ignore=tests/test_simulation_pipeline.py
```

**Run a single test file:**
```bash
python -m pytest tests/test_batter_model.py -v
```

**Run a single test:**
```bash
python -m pytest tests/test_batter_model.py::TestBatterPCAModel::test_fit_returns_self -v
```

**Run the simulation pipeline integration test** (requires built copula first):
```bash
python scripts/build_copula.py
python tests/test_simulation_pipeline.py
```

### Data flow

```
DK Salary CSV  ──► DraftKingsSlateIngestor ──► players_df (player_id, position, salary, team, game, slot)
                                                        │
Slate/Player Exclusions ────────────────────────────────► prune players_df
                                                        │
Projections CSV ──┐                                     │
RotoWire API  ────┼──────────────────────────────────── ► players_df.mean / std_dev
Salary heuristic ─┘                                     │
                                                        ▼
Retrosheet EVN ──► process_historical.py  ──► historical_logs.parquet
                                                        │
                             ┌──────────────────────────┤
                             ▼                          ▼
                     build_copula.py           fit_batter_pca.py
                             │                          │
                             ▼                          ▼
              empirical_copula.parquet    batter_pca_model.npz + batter_score_grid.npy
                             │                          │
                             └──────────┬───────────────┘
                                        ▼
                              SimulationEngine.simulate(n_sims)
                                        │
                                        ▼
                              SimulationResults  (n_sims × n_players matrix)
                                        │
                                        ▼
                   CandidateGenerator.generate()  [n_candidates stacked lineups]
                                        │
                                        ▼
                   ContestScorer.score_candidates()  [K simulated fields × M candidates]
                                        │
                                        ▼
                   DeterminantPortfolioSelector.select()  [portfolio_size lineups]
                                        │
                                        ▼
                               Portfolio (n lineups)
                                        │
                          [Optional entry file workflow]
                                        ▼
                   parse_entry_file() → assign_lineups_to_entries()
                        → write_upload_files()  (upload_*.csv for DK)
```

### The copula unit

Players are grouped by `(team, opponent)` into 10-player "units": the 9 batters of one team plus their opposing pitcher. The empirical copula is a `G × 10` matrix of historical rank-quantiles (one row per historical game-team observation). Simulation samples at the **game level**: `EmpiricalCopula.sample_games()` bootstraps whole historical games and assigns the two paired rows to the game's two opposing units, preserving the correlation between a team's batters and its own pitcher (~+0.10 at the quantile level) and the shared run environment. Units without a partner on the slate (or copulas lacking a `(game_id, team_id)` index) fall back to independent row sampling via `sample()`.

Sampled rows then pass through a **sim-time dependence overlay** (`_apply_env_overlay` in `src/models/copula.py`): a shared per-unit Gaussian factor with calibrated loadings (`_ENV_OVERLAY_GAMMA` for batters, `_ENV_OVERLAY_DELTA` for the opposing pitcher) rotates the Gaussianized ranks so the joint dependence matches *realized residual* dependence rather than the rows' unconditional historical dependence — the raw rows double-count identity/matchup effects that the matchup-conditioned marginals already price (batter-batter +0.13 → target +0.19; batter-vs-opp-pitcher −0.335 → target −0.18). Marginals stay exactly uniform; the two rows of a game get independent factors (cross-team corr ≈ 0 is preserved). Calibrated by `scripts/diagnose_sim_tails.py` (PIT calibration of replayed sims against realized FPTS across the contest archive) — re-fit as the archive grows.

### Marginal distributions

Precedence per player (highest first):

1. **Market-implied quantile grid** (`EmpiricalQuantileMarginal`) — when the market-odds fetcher produced a validated 101-point percentile grid for the player (`data/processed/projections_mo_dist.parquet`, built by a per-player Monte Carlo over the fitted market rates with structural couplings: R ≥ HR, RBI ≥ HR, K ≤ outs, 5-IP win rule, early-exit outs mixture; batter event rates share a mean-one lognormal run-environment factor `_ENV_SIGMA`, calibrated against Retrosheet box-score co-movement by `scripts/calibrate_env_sigma.py`). Market-implied means carry an **empirical mean calibration** (`_MEAN_CALIB_BATTER`/`_MEAN_CALIB_PITCHER` in the fetcher, fitted 2026-07-06 against realized FPTS across the archive: batters ×0.867, pitchers ×0.946 — residual vig the fair-odds devigging doesn't remove). Batters are deflated at the *rate* level (mean scales linearly, spread √-style, couplings intact); pitchers post-hoc on the whole marginal. RotoWire-fallback players are not scaled. Archives from 2026-07-06 onward contain calibrated means — account for this when re-fitting. Grids are matched defensively: applied only when the player's projected mean agrees with the grid mean within ±0.5 (`src/models/quantile_grids.py`), so fallback-sourced or stale entries keep the parametric path.
2. **Batters (slots 1-9)**: `BatterMixtureMarginal` — mixture of Exp(λ) + N(μ,σ). At runtime, `BatterPCAModel.project(mu_proj, sigma_proj)` solves a 2×2 linear system to find the point on the fitted PCA plane that satisfies the projection constraints, yielding the full `(w, λ, μ, σ)` parameter set.
3. **Fallback (pitchers without a grid; batters without a PCA model)**: `GaussianMarginal(mu, sigma)` — direct `scipy.stats.norm.ppf`.

`scripts/compare_marginals.py` reports before/after moment and percentile shifts (synthetic archetypes, or per-player when a dist parquet is present).

### Portfolio construction

Three stages in `src/optimization/`:

- **`CandidateGenerator`** (`candidate_generator.py`) — generates a large pool of stacked candidate lineups (default 20k). Uses ownership-weighted sampling to produce correlated batter stacks with matching pitchers. **Sim-winner seeding** (`generate_sim_winners`, behind `gpp.seed_sim_winners` + `gpp.n_sim_winner_worlds/sim_winner_per_world/sim_winner_temp/sim_winner_own_blend`): for each of many stratified sim worlds, draws lineups through the same `_sample_one` machinery with per-world rank-softmax weights over that world's realized scores — the scaled, sampling-based successor to per-sim exact ILP seeding (`seed_sim_optimal_lineups`). Dumped as `seed_source="sim_winner"`. **Shape-preserving seed mutation** (`generate_shape_mutants`, behind `gpp.seed_mutants_per_parent` + `gpp.seed_mutant_salary_locality/seed_mutant_pitcher_weight`): expands each seed parent (sim_optimal + sim_winner) with N mutants whose team-stack profile matches the parent *exactly* — batter slots swap only within the same team (swap-in must cover the outgoing primary position), pitcher slots swap conflict-checked against the rostered hitters; replacement sampling is salary-local. Built for seed-first pools (round 6): `refine_rounds: 0` gives the best pool but starves the Det selector of neighborhood diversity, and the generic `generate_mutants` refinement mutants restore it only by breaking seed shape. Additive on top of `n_candidates`, dumped as `seed_source="seed_mutant"`. Parents are always cut into fixed-size chunks (`_SHAPE_MUTANT_CHUNK`), each drawing from its own `SeedSequence` child and each starting from the same `seen` snapshot, so the optional `n_workers` (0 = auto — physical cores minus one, affinity aware, capped at the chunk count; 1 = serial) is a pure speed dial — the mutants are identical at any worker count, and cross-chunk collisions are dropped at merge rather than re-drawn. Processes, not threads: the loop is pure Python. Used by `mrp.frontier_lineups` via `marginal_reward.frontier_mutant_workers`, where mutation was the largest block of the stage (measured 08/25: stage 39.7s → 22.1s, mutation 21.8s → 4.5s on 7 workers).
- **`ContestScorer`** (`gpp_portfolio.py`) — scores each candidate against K simulated opponent fields (drawn from `ContestSimulator`), producing a `robust_payout` matrix (M candidates × n_sims). A Numba JIT kernel computes per-lineup payouts against the sorted field with exact tie splitting (two binary searches bound the tie band; payout = prefix-sum mean over it). Optional duplicate-entry penalty (`gpp.dupe_penalty`): a log-linear model (Σ log ownership, unused salary, primary stack size — coefficient knobs in `gpp.dupe_*`, fitted by `scripts/fit_dupe_model.py` as a zero-truncated Poisson GLM on the archived contest standings; re-fit as the archive grows) estimates E[dupes] per candidate and dilutes top payout bands (gross ≥ `gpp.dupe_min_gross_payout`) by 1/(1+E[dupes]).
- **Fresh re-score** (`ContestScorer.rescore_fresh_fields`, wired in `pipeline.py`) — after refinement, every candidate with mined EV ≥ `gpp.ev_floor` is re-scored against `gpp.final_n_field_samples` freshly drawn fields (disjoint seeds), then candidates whose **fresh** EV falls below the floor are discarded — a lineup must clear the floor twice to reach the selector. This kills the EV winner's curse from mining on the first-stage fields while keeping the low-EV/high-diversity band above the floor available to the diversity-weighted selector (an EV-rank cutoff here would starve Det-EV of diverse picks). `gpp.final_rescore_top` is only a safety cap on the slice size (memory/time guard — a warning logs when it binds). Set `final_n_field_samples: 0` to disable. **Tail bypass**: the top `gpp.tail_bypass_n` *below-floor* candidates by per-candidate sim-p99 (`_candidate_sim_tail_scores` — a ceiling statistic that mean-dollar EV systematically undervalues) are also admitted to the fresh re-score; they must keep fresh EV ≥ `gpp.tail_bypass_ev_floor` to survive, and the selector's backstop cull is lowered to that floor when any survive. Bypass candidates are marked `tail_bypass=1` in the candidate-pool dump for post-contest evaluation. Set `tail_bypass_n: 0` to disable.
- **Tail metrics + funnel modes (ceiling-first redesign)** — with `gpp.compute_tail_metrics` on, `ContestScorer` also accumulates per-candidate **tail_ev** (expected gross dollars from payout ranks paying ≥ `gpp.tail_ev_min_gross` only — the steep band, blind to the min-cash plateau that dominates mean EV) and **p_beat99** (fraction of sims beating the simulated field's p99), in both mined and fresh flavors; all land in the pool dump (`sim_p99, tail_ev, p_beat99, fresh_tail_ev, fresh_p_beat99`). `gpp.funnel_mode`: `ev_first` (EV floor primary; tail lane = `tail_bypass_n` side door) or `tail_first` (top `gpp.tail_admit_n` by `gpp.tail_metric` admitted to the fresh re-score, held only to `gpp.ev_guardrail`; the EV-floor lane persists as the cash-anchor source). `gpp.tail_metric` picks the lane currency: `tail_ev` / `p_beat99` / `sim_p99`.
- **Per-contest selection** (`src/optimization/multi_contest.py`) — when the slate has entry files, the contests in them are grouped, each ladder resolved from the **advertised prize pool** in the contest name (not entries back-solved as pool/fee, which is wrong by the rake and picked the wrong size variant for 5 of 6 real contests), and each contest selects **its own slice against its own ladder and field size**, in descending-top-prize fill order. This is the objectives' largest lever: the same pool produces portfolios ~19 ownership points chalkier for a 235-entry contest than for an 11,437-entry one, unprompted. **Nothing is typed**: contest name, entry fee, advertised prize pool and implied field size are all on every row of a DK entries file, so `gpp.contest_structure` / `contest_field_size` are now an OVERRIDE used only when set or when there is no entries file (replay, ad-hoc). Resolution and the approximate-ladder confirmation gate run **before generation**, not after the pool is built. The **funnel** (EV floor, fresh re-score, tail metrics) still needs one ladder and field size, derived by `_funnel_reference` as the contest with the most money at risk (entries x fee) — the rule lands near the $4/~5,000-entry reference every downstream threshold was calibrated against, whereas 'largest field' would anchor a typical file on a $1 contest where a $0.20 `ev_floor` is a 20% ROI hurdle. Because any single reference biases WHICH lineups survive and not merely how many, the shortlist is built by **ranking the whole pool against every contest's own ladder and unioning the results** (`_union_shortlist_by_contest` -> `multi_contest.union_shortlist`): contests take turns contributing their next-best not-yet-chosen candidate, which deepens shared coverage when they agree and fills with each contest's own preferences when they disagree. Measured on a real 15k pool, Spearman(mean EV, ownership) = +0.356 against a 5,945-entry reference, and a plain top-4,000 cut took 118 of 1,500 from the least-owned decile against 603 from the ninth — a 5:1 skew toward chalk, worst exactly where contests diverge. Ranking runs at REDUCED world resolution (`gpp.per_contest_rank_sims`, default 2,500 of 25,000): picking a menu is coarse work, and the full-resolution pass costs ~98s against ~10s. Its field is drawn on `_RANK_FIELD_SEED_OFFSET`, disjoint from the selection pool, so the shortlist is never ranked on the field it will be priced against. `per_contest_rank_sims: 0` falls back to `_stratified_shortlist` — equal-population ownership bands, best-by-EV within each — which is the same idea proxied through one assumed axis (measured: full decile coverage for 3.2% of mean EV). The single-ladder path is still the whole story when there are no entry files — which is why `scripts/replay_slate.py` stays reproducible (archives carry no `*Entries.csv`). Contests are the **outer** loop (`select_per_contest_multi_arm`): building a contest's sorted field and running the payout kernel depends on the contest alone, so arms-on-the-outside would rebuild all of it per arm (36 field builds instead of 6 at six contests × six arm keys). One field pool is generated at the largest contest's size and **subsampled** per contest (`SharedFieldPool`) — field entries are i.i.d. draws from the same ownership model, so an F-subset is a valid F-field, and nested subsets give common random numbers so the cross-contest ownership spread is a ladder effect rather than field noise. Per-contest payouts run the production kernel (`contest_payout_matrix`) with the dupe model **rescaled by field size** (`scale_dupes_for_field`; E[copies] is linear in field size and `ContestScorer` never rescales because its whole run is one contest). Each arm keeps its own `used` set so slices are disjoint **within** an arm (measured: +4.1 pts P(at least one contest profits), +2.9 P(at least one doubles), at 5.7% of mean EV) but arms never constrain each other. Every arm flattens in the same order — contests in fill order, entries within a contest in file order — so one entry map serves the whole sweep; assignment is positional (`_assign_positional`), not `assign_lineups_to_entries`, and the diversity reorder is skipped since it exists to align lineup strength with entry *fee* across a fee-heterogeneous slate. Knobs: `gpp.per_contest_shortlist`, `per_contest_field_samples`, `per_contest_disjoint`, `per_contest_shortlist_strata`, `per_contest_cand_chunk`. **`per_contest_shortlist` is a computational guard, never a quality device** — a smaller value only removes options the arms would otherwise have had — so it is sized against the budget and against the AUGMENTED pool, not the imported one: an import (~4,200) plus `frontier_target_lineups` (4,000) lands near 8,200, which the old 4,000 default cut by more than half, undoing the frontier that had just run to widen the menu. Default 8,000; at 8,200 in the pool the `_PC_RANK_MIN_CUT` shortcut then fires and the union-rank stage is skipped outright. Measured 2026-08-27 by `scripts/bench_per_contest_shortlist.py` at n_sims=25,000 on the 08/25 six-contest entries file (fields 792-17,835): cost is LINEAR in M and memory is set by the single largest contest. Peaks are measured on that contest, wall figures extrapolated to all six from the two largest (the four small ones are dominated by the field-size-insensitive dR state build):  4,000 -> 3.1GB/~3.5min, 8,000 -> 4.3GB/~7min, 12,000 -> 5.6GB/~10min, 16,000 -> 6.9GB/~13min. The dR state build (`marginal_reward.precompute_field_ranks`) was 55% of that marginal cost and 41% of a whole 544s run until it was made a `@njit(parallel=True)` kernel over the world axis (2026-08-27): measured 39.3s -> 6.1s, **6.4x**, at M=8,202/S=25,000/F=5,945. Output is bitwise-identical for finite scores and `_precompute_field_ranks_numpy` is kept solely so a test can keep asserting that. Worlds are the parallel axis, not candidates: `field_sorted[s]` is the array being searched (71KB at F=17,835, L2-resident across all M candidates of that world), whereas splitting over candidates would stream a fresh row per iteration. A run that keeps under half its pool logs a WARNING naming the knob, because over-trimming here is otherwise silent. Per contest, `per_contest_cand_per_entry` (default 400, floor `_PC_CAND_CAP_FLOOR` = 2,000, 0 disables) then caps how many of those candidates THAT contest chooses from, applied to every arm identically after the used-set exclusion so the arms stay comparable. A contest's cost is (cap x n_sims) regardless of how many lineups it needs, and measured 08/27 three 2-entry contests spent 96s of dR state over 8,202 candidates to choose six lineups — 22% of the selection stage. It is safe exactly where it bites: dR's first pick IS argmax of that contest's mean EV and every later pick is that minus a demotion term the earlier picks create, so at k=2 there is one demotion pair in the whole problem; at large k that term does real work on the contrarian tail an EV ranking cuts first (Spearman(mean EV, ownership) = +0.356), which is why the rate is per ENTRY and 400 is a no-op above ~20 entries on a typical augmented pool. Unlike the parallel dR state build it DOES change which lineups are picked — a speed/quality trade, not a free one. **Also reachable from External Pool mode** as `gpp.external_pool_ev_type: per_contest`, which runs the imported SaberSim pool through this same machinery with `gpp.selector_mode`'s arms — the external path already supplies every input it needs (`pool.lineups`, `compute_lineup_scores` for the (M, n_sims) matrix, mandatory entries files, `ContestSimulator` for the field). It is a sibling of `marginal_reward`, not a replacement: both price self-competition per contest and both use `mrp/delta_reward.ContestDeltaState` for dR, but MRP runs ONE global greedy over (lineup, contest) pairs with `gamma_in`/`gamma_out` overlap caps, while this fills each contest's slice in descending-top-prize order with `exclude_used` disjointness. Which shape wins is open and measurable. **The imported pool is a starting point, not the whole candidate list**: `marginal_reward.frontier_enabled` now drives the line-2 mean-variance generator on this path too (`PipelineRunner._frontier_augment_pool`), merging generated lineups into `pool.lineups` before the shortlist so the arms choose from more than the export. It reads the same `marginal_reward:` block MRP reads — one flag should mean one thing across the two sibling shapes — and is driven from `ContestSlot`s via `runner.frontier_contests_from_slots`, which passes each slot's ALREADY-RESOLVED ladder through rather than re-looking it up by name (`nearest_payout_structure`'s entry-count match is the rake-blind one that picked the wrong variant for 5 of 6 real contests). The field pool is built BEFORE the shortlist for this — sigma_dG needs a simulated field and the generated lineups must exist before anything is ranked — and is scored at `max_sims_per_contest` worlds for the generation-side estimate, since the (S x F) array is this stage's memory peak (1.8GB at 25,000 x 17,835). Augmentation NEVER removes an imported lineup, and fails soft-but-loud: a slate that cannot supply what the generator needs leaves the pool alone and logs at WARNING, because a silent no-op here is exactly how a real run came to fill 110 entry slots from 4,219 SaberSim lineups (a 2.6% cut, against 0.4% on the generated path) with `frontier_enabled: true` sitting in config doing nothing. Provenance reaches the UI as `from_generated` (the `GEN` badge and the "N of M came from the generated frontier" counter) via `_external_generated_keys` — a set of `frozenset(player_ids)`, not the positional list MRP uses: per-contest selection produces one portfolio PER ARM off a shared pool, so entry slot 7 holds a different lineup in every arm and only a lineup-keyed verdict can describe them all. The keys are exact rather than a heuristic because `_frontier_augment` drops any generated lineup whose frozenset already exists in the imported pool, so no lineup is both. `_build_external_entry_map` omits `from_generated` whenever the key set is present (entry meta is merged ONTO already-serialized rows and would otherwise overwrite a per-arm verdict with a slot-indexed one), and both are cleared at the top of every `_run_external` so a later generator-off run cannot inherit the verdict and label imported lineups as generated. Measured 08/27 on the 08/25 slate: generated lineups were 49% of the pool and took **85% of the 110 entry slots** (84-87% across all six arms). **Any figure averaged across contests is blended by construction** — a portfolio spanning a 792-entry Pickoff (mean own 115.8) and a 17,835-entry mini-MAX (89.0) averages to ~94, which describes neither and hides the spread that is the whole point; the UI reports per contest and shows the range, never the mean.
- **`DeterminantPortfolioSelector`** (`gpp_portfolio.py`) — greedy portfolio assembly via incremental positive-redundancy-distance maximization. Scores each successive lineup on `EVw·EVn + DEw·Dn + HedgeW·HedgeN`: `Dn = 1 - Σᵢ max(rᵢ, 0)²` (clipped to `[0, 1]`), summing the squared correlation to *each* already-selected lineup `i`, counting only positive (co-moving) correlations — negatives are floored to 0 before squaring/summing so a hedge to one already-selected lineup can never cancel out a near-duplicate of another. Squaring (not summing raw `r`) correctly prefers one strong overlap over several moderate ones (true incremental coverage against an orthogonal existing portfolio is `1 - Σr²`, not `1 - Σr`), and unlike a naive aggregate correlation to the portfolio's summed payout, it never dilutes as the portfolio grows — an irrelevant already-selected lineup contributes exactly 0, not a shrinking share of an average. Since flooring negatives out of `Dn` means a hedge and an uncorrelated lineup would otherwise tie, `HedgeN = min(Σᵢ max(-rᵢ, 0)², 1.0)` restores a bounded hedge credit as a separate term rather than folding it back into `Dn` (which would reopen the cancellation hole `Dn` was built to close). `DEw`/`HedgeW` split off `(1-EVw)` as `0.9×(1-EVw)` / `0.1×(1-EVw)` (`_HEDGE_WEIGHT_FRACTION`) so `HedgeW ≤ DEw` always — the capped-at-1.0 hedge bonus can never outscore a fully non-redundant candidate's guaranteed `Dn=1` floor, so it can only break ties among non-redundant candidates, never rescue a redundant one. `gpp.selector_score: tail` makes the EV term rank by the fresh tail currency (`ev_override`), with the first `ceil(gpp.cash_anchor_fraction × size)` picks still on mean dollar EV; the floor cull, diversity term, and reported EVs stay on mean dollar EV.

### Offline replay harness

`scripts/replay_slate.py` re-runs the full production pipeline on an archived slate's inputs (`archive/MMDDYYYY/DKSalaries.csv` + `market_odds_projections.csv`; team totals resolve automatically from the slate date; live exclusions/twitter scratches are inherited via the DKSalaries fingerprint) and grades every funnel stage — pool, floor set, fresh survivors, seed blocks, per-risk portfolios, arbitrary `--rank-col` slices — against the real contest field from the standings zip (hit99/hit99.9, cash rate, realized net $, per-column Spearman). Batch mode: `--recent N` / `--all` × `--variants <yaml>` (pre-registered matrices — see `outputs/replay/variants_phase123.yaml`), results appended to `outputs/replay/replay_summary.csv`. `--screen` = 10k sims/10k candidates coarse profile. Sims are cached per (slate, n_sims, seed, projections-hash) in `outputs/replay/sim_cache/` so config variants reuse one sim; `PipelineRunner(persist_caches=False, sim_cache_path=...)` are the replay-only hooks (never set by the live server). Pre-2026-07-06 archives get the mean calibration applied at load (`--no-precalib` disables). Fidelity gate (2026-07-12, 4 post-overlay slates): replayed pool hit99 pooled 1.05% vs live 0.98% — judge variants by deltas.

`scripts/fit_winner_shape.py` + `src/optimization/winner_shape.py` — sim-free logistic on real top-1% entry composition (walk-forward artifact `data/processed/winner_shape_model.json`). Measured weak (mean walk-forward AUC 0.554, 2026-07-12): composition alone barely discriminates the real ceiling; kept for reference, not wired as a funnel currency.

### Payout functions

`src/optimization/payout.py` provides `load_payout_structure()` and `payout_table_to_array()` for loading the reference GPP payout structure from `data/payout_structures/dk_classic_gpp.json`. Used by `ContestScorer` and `ContestSimulator` to compute per-rank net payouts.

### Entry file workflow

`src/api/dk_entries.py` supports the DraftKings upload workflow:
1. Parse an existing DK entry CSV with `parse_entry_file()` → list of `EntryRecord`.
2. Assign optimized lineups to entries with `assign_lineups_to_entries()`.
3. Write DK-ready upload CSVs with `write_upload_files()` → `outputs/upload_*.csv`.

### Slate and player exclusions

`src/api/slate_exclusions.py` manages persistent exclusions stored in `data/slate_exclusions.json`. Excluded games/teams and players are pruned from `players_df` before simulation. Server endpoints: `POST /api/slate/exclusions` and `POST /api/slate/player-exclusions`.

### Key data contracts

`players_df` passed to `SimulationEngine` and `BasinHoppingOptimizer` must have columns:
`player_id, team, opponent, slot, mean, std_dev, position, salary, game`

`SimulationResults` core properties: `results_matrix` (ndarray, shape `n_sims × n_players`), `player_ids` (list, column order matches matrix).

### Precomputed vs. runtime artifacts

| Artifact | Script | Used by |
|---|---|---|
| `data/processed/historical_logs.parquet` | `scripts/process_historical.py` | `build_copula.py`, `fit_batter_pca.py` |
| `data/processed/empirical_copula.parquet` | `scripts/build_copula.py` | `SimulationEngine` |
| `data/processed/batter_pca_model.npz` | `scripts/fit_batter_pca.py` | `SimulationEngine` |
| `data/processed/batter_score_grid.npy` | `scripts/fit_batter_pca.py` | `SimulationEngine` |
| `data/processed/ownership_calibrator.json` | `scripts/fit_ownership_calibrator.py` | pipeline ownership calibration (`load_ownership_calibrator`); stale-checked against the `ownership.py` constants hash — re-fit after any constants change |
| `data/slate_exclusions.json` | runtime (API) | `slate_exclusions.py` |
| `data/payout_structures/dk_classic_gpp.json` | manual / reference | `payout.py` (calibration) |

### Web UI and API

The web app is a React + TypeScript + Vite frontend (`ui/`) backed by a FastAPI server (`src/api/server.py`).

**Key API endpoints:**
- `GET /api/config` / `POST /api/config` — read/write `config.yaml`
- `GET /api/projections/fetch` — stream projections fetch output (SSE); `GET /api/projections/status` — freshness check. `paths.projections_source: sabersim` is a local-file import rather than a network fetch: it reads the newest `data/raw/MLB_*_DK_*.csv` (the same SaberSim-style export used by External Pool mode, see `src/api/external_pool.py::parse_sabersim_projections`) instead of running a fetch script. `mean` comes from the file's `My Proj` column on DK (the user-edited projection, the same column external-pool mode reads via `parse_player_projections`, falling back to `dk_points` where `My Proj` is blank) or `fd_points` on FD (`My Proj` is DK-scored); `std_dev` comes from `dk_std`/`fd_std`; `lineup_slot` comes from `Order` for hitters and is fixed at `10` for pitchers. Pitchers ignore `Status` entirely (the export lists the whole staff, not just the starter, and `Status` there isn't a reliable confirmed-starter signal): the pitcher with the highest projected mean on each team's roster is kept as that team's starter and is always given `slot_confirmed = True`, so an external-projection pitcher never counts toward the unconfirmed-slot total (portfolio panel included) regardless of its actual confirmation status. A hitter row is kept whenever it has an `Order` regardless of confirmation, and `slot_confirmed` for hitters is `Status == "Confirmed"` (projected-but-unconfirmed slots still show as the amber bubble, confirmed as green). Ownership is **not** computed by `compute_heuristic_ownership` for this source — it's read straight from the file's `Adj Own` column (as a fraction) and carried through `players_df["ownership"]`, which `PipelineRunner.run()` uses in place of the heuristic model wherever present. SaberSim is the sole confirmed-lineup source of truth when active: Twitter/Underdog-parsed lineups are not applied on top of it, in the pipeline (`PipelineRunner.run()` skips `_apply_twitter_overrides()`), in `GET /api/projections/players|status|unconfirmed` (skip the Twitter-confirmed merge/count-adjustment), and in the UI (`ProjectionsTable.tsx`'s `isSaberSimSource` prop forces every team's locked-lineup card, lock button, and per-team "Refresh from RotoWire" button off, since a stale or pre-existing Twitter lock would otherwise keep rendering its own snapshot instead of the CSV's current data — this was the actual bug behind a player with no `Order` still showing as confirmed). SaberSim being the source of truth for the *pipeline/UI* doesn't mean Twitter/Underdog-parsed lineup updates are ignored, though: `save_twitter_lineup` (`src/api/server.py`) always diffs an incoming Twitter update against the live SaberSim-confirmed baseline (`_best_guess_lineup_slots`, restricted to `slot_confirmed` rows and including the pitcher at `lineup_slot == 10`) rather than against a previously-locked Twitter record, with no first-pitch timing gate — so a contradicting scratch/pitcher-swap/lineup-change notification emails and lands in the Slate tab's notification list at any time of day, not just near lock. The pitcher is included in `parse_notification_body`'s parsed slots (as `slot: 10`, preferring an `SP` line over `RP`/opener lines) and in the diff, called out on its own "Pitcher:" line in the email body since it's usually the most fantasy-relevant change. The raw dbus scratch-keyword email trigger (`_maybe_commit_notification`) gets the same SaberSim timing bypass.
- `GET /api/projections/players` — current player projections; `GET /api/projections/team_totals`, `GET /api/projections/unconfirmed`, `GET /api/projections/merge_info`
- `GET /api/run/stream` — SSE stream of pipeline progress events
- `GET /api/run/status` / `POST /api/run/stop` — run state management
- `GET /api/portfolio` — last completed portfolio (persisted across restarts)
- `GET /api/portfolio/sweep` — risk-sweep results; `POST /api/portfolio/activate_risk` — select a risk tier; `POST /api/portfolio/replace/{lineup_index}` — swap one lineup
- `GET /api/contest/analyze` — contest analysis / field simulation
- `GET /api/slate/games` / `POST /api/slate/exclusions` — game exclusions
- `GET /api/slate/players` / `POST /api/slate/player-exclusions` — player exclusions
- `GET /api/slate/ownership-reductions` / `POST /api/slate/ownership-reductions` — per-player ownership fade
- `GET /api/slate/projection-overrides` / `POST /api/slate/projection-overrides` — per-player mean overrides (`dict[int, float]`; no std override exists). Applied to `players_df["mean"]` in `PipelineRunner._apply_exclusions()` before exclusion filtering, so both `sim_players_df` and `cand_players_df` (and therefore the optimal-lineup ILP solver) see the overridden mean.
- `GET /api/notifications` / `DELETE /api/notifications/{id}` — in-app notification log
- `GET /api/twitter-lineups` / `POST /api/twitter-lineups` / `DELETE /api/twitter-lineups/{team}` — confirmed opponent lineup tracking
- `POST /api/lineups/{team}/lock` / `DELETE /api/lineups/{team}/lock` — lock/unlock a lineup
- `GET /api/late-swap/state` / `POST /api/late-swap/run` / `POST /api/late-swap/override` / `POST /api/late-swap/reset` — late-swap workflow

**Pipeline progress events** emitted via SSE: `load_slate`, `simulate`, `compute_target`, `gpp_generate_start/progress/done`, `gpp_score_start/done`, `gpp_optimal_start/progress/done`, `gpp_sim_optimal_start/progress/done`, `gpp_seed_mutant_start/progress/done`, `gpp_refine_start/progress/done`, `gpp_rescore_start/field_progress/score_progress/done`, `gpp_per_contest_start`, `gpp_contest_field_progress`, `gpp_contest_select`, `gpp_contest_done`, `gpp_per_contest_done`, `portfolio_stats`, `complete`. The rescore field/score progress events are named distinctly from the first-stage `gpp_field_progress`/`gpp_score_progress` (same payload shape, different stage string) so the UI can show a live Fresh re-score readout instead of a stale first-stage label held over in `ProgressPanel.tsx`. `gpp_contest_field_progress` is named distinctly from both for the same reason. Per-contest selection reports at CONTEST granularity, not per arm or per pick: `gpp_contest_select`/`gpp_contest_done` bracket each contest and carry `work_done`/`work_total`, where work is `field_size + shortlist` — a proxy blending the two dominant terms (field sort O(F log F) per world, payout kernel O(M log F) per world) that the UI calibrates against measured elapsed time for the ETA. Contests fill in top-prize order, not size order, so a flat contests-done/total bar would lurch. `gpp_contest_done` and `gpp_contest_field_progress` are suppressed from the event log (bar/ETA only); the log gets one row per contest plus the phase start/done pair. The `complete`/`stopped` payloads carry a `contests` list (one row per contest: identity, resolved table, lineup count, mean ownership/EV) whenever per-contest selection ran.

**UI components** (`ui/src/components/`): `ConfigForm`, `SlatePanel`, `ProjectionsPanel`, `ProjectionsTable`, `ProgressPanel`, `PortfolioTable`, `MetricsPanel`, `TeamBadge`, `StopUploadDialog`, `LateSwapPanel`, `LineupParserDialog`, `RunOptionsDialog`, `DeleteConfirmModal`.

Team logos for all 30 MLB franchises are in `ui/public/team-logos/`.

**Adding new config fields:** any field exposed in the UI (`ConfigForm.tsx` / `ui/src/types.ts` `GppConfig`) must also be declared in `src/api/models.py` `GppConfig`. Pydantic silently drops unknown fields on `POST /api/config`, so omitting a field there means Save Config is a no-op for that field. The three places that must stay in sync: `config.yaml` (default value), `src/api/models.py` `GppConfig` (Pydantic model + default), `ui/src/types.ts` `GppConfig` (TypeScript type).

## DK Classic GPP payout structure

`data/payout_structures/dk_classic_gpp.json` models a typical DK $4 Classic GPP: 14,863 entries, 3,855 paying positions (~26% cash rate). The cash cutoff is finishing in roughly the **top 26%**, which requires beating ~74% of the field. `ContestSimulator.eval_portfolio()` reads this file via `PipelineRunner._load_cash_threshold()` to derive the `cash_threshold` used in `cash_rate` computation — do not use `> 0.5` (beats half the field) as a cash proxy.

## CrazyNinjaOdds market names (confirmed)

The market dropdown option labels in `fetch_market_odds_projections.py` have been verified against the live site's `<select>` element. **Do not change these strings** — they are confirmed correct:

- `"Player Batting Walks"` (confirmed 2026-04-26; the dropdown also contains `"Player Batting Strikeouts"` as a distinct adjacent entry — do not conflate or rename either)

If a market appears to fail silently, investigate AJAX timing or sub-market ("All players") selection before assuming the label is wrong.