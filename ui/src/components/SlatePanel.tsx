...
export function SlatePanel({ disabled, projFetchExcluded = [], onProjFetchFilterChange, platform = 'draftkings', notifications = [], onDismissNotification, twitterLineups = [], onParseNotification }: Props) {
  ...
  return (
    <div className="slate-panel">
      ...
      {(() => {
        const confirmedTeams = new Set(twitterLineups.map(tl => tl.team))
        const pending = notifications.filter(n => !n.lineup_team || !confirmedTeams.has(n.lineup_team))
        ...
        return (
          <div className="twitter-notifications">
            <div className="twitter-notifications-header">
              ...
              <span className="slate-summary">{pending.length} unread</span>
            </div>
