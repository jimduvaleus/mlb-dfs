...
export function PortfolioTable({ lineups, optimalLineups = [], portfolioSweep = [], activeRisk = 1, onActivateRisk, unconfirmedPlayerIds, onDeleteLineup, replacingLineupIndex, platform, evwBase = 0.10, evwMax = 0.40 }: Props) {
  ...
  // meaningless "0 of N".
  //
  // Per ARM, not per run: per-contest selection builds a separate portfolio for
  // each arm off one shared pool, and they do not agree on how much generated
  // material to take (measured 84-87% across six arms on 08/25). This reads
  // `activeLineups`, so it answers for whichever arm is on screen.
      ...
      <div className="portfolio-tabs">
        ...
        {nGenerated > 0 && activeTab === 'portfolio' && (
          <span
            ...
            title="Lineups built by the line-2 mean-variance frontier rather than imported from the SaberSim pool. Counted for the ACTIVE arm — each arm picks its own, so this moves as you switch."
          >
                ...
                <div className="lineup-card-entry-info">
                  ...
                  {lineup.from_generated && (
                    <span
                      ...
                      title="Built by the line-2 mean-variance frontier, not imported from the SaberSim pool"
                    >
                      GEN
                    </span>
                  )}
